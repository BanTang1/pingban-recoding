...
class AllAppViewModel : ViewModel() {
    ...
    // 服务器获取三方Apk 名称、图标等信息
    private var isPadApkInfoLoadSuccess: Boolean = false

    // 服务器获取三方APK列表
    private var isPadApkListLoadSuccess: Boolean = false

    fun getPadApkList(type: String) {
        uiScope.launch {
            do {
                isPadApkInfoLoadSuccess = false
                withContext(Dispatchers.IO) {
                    realGetPadApkList(type)
                }
            } while (!isPadApkInfoLoadSuccess)
        }
    }
    ...
    fun initApk() {
        uiScope.launch {
            do {
                isPadApkListLoadSuccess = false
                withContext(Dispatchers.IO) {
                    realInitApk()
                }
            } while (!isPadApkListLoadSuccess)
